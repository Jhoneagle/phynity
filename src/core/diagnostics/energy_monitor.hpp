...
public:
	...
	void update(double current_energy)
	{
		...
		if (change < 0.0)
		{
			...
			{
				trigger_violation(
				    {frame_number_, previous_energy_, current_energy, std::abs(change), loss_percent, "loss"});
			}
		}
		else if (change > 0.0)
		{
			...
			{
				trigger_violation({frame_number_, previous_energy_, current_energy, change, change_percent, "gain"});
			}
		}
