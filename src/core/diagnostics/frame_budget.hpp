...
public:
    ...
    void check_frame(const FrameProfile &frame)
    {
        ...
        {
            trigger_violation({"", // Empty name = total frame
        ...
        for (const auto &[zone_name, budget] : zone_budgets_)
        {
            ...
            for (const auto &zone : frame.zones)
            {
                ...
                {
                    trigger_violation({zone.name, budget, zone.duration_us, frame.frame_number});
    ...
    void check_recent_frames(const FrameProfiler &profiler, size_t num_frames = 1)
    {
        ...
        const size_t frames_to_check = std::min(num_frames, static_cast<size_t>(profiler.get_frame_count()));
