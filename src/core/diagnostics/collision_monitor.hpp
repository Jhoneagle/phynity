...
public:
	...
	void end_frame()
	{
		...
		{
			efficiency =
			    static_cast<double>(current_actual_collisions_) / static_cast<double>(current_broadphase_candidates_);
...
private:
	void reset_frame_counters() noexcept
	{
		...
		if (current_broadphase_candidates_ > 0)
		{
			last_efficiency_ =
			    static_cast<double>(current_actual_collisions_) / static_cast<double>(current_broadphase_candidates_);
			last_false_positive_rate_ =
			    static_cast<double>(current_broadphase_candidates_ - current_actual_collisions_) /
			    static_cast<double>(current_broadphase_candidates_);
		}
		else
		{
