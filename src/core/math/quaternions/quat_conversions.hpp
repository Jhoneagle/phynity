...
template <typename T = float> inline Mat3<T> toRotationMatrix(const Quat<T> &q)
{
...
template <typename T = float> inline Quat<T> toQuaternion(const Mat3<T> &m)
{
    ...
    bool nearOrtho = std::abs(m.getRow(0).dot(m.getRow(1))) < rotation<T>::orthonormal_epsilon &&
...
template <typename T = float> inline Vec3<T> toEulerAngles(const Quat<T> &q)
{
...
template <typename T = float> inline Quat<T> toQuaternion(const Vec3<T> &euler)
{
    T roll = euler.x;
    T pitch = euler.y;
    T yaw = euler.z;
...
template <typename T = float> inline void toAxisAngle(const Quat<T> &q, Vec3<T> &axis, T &angle)
{
    Quat<T> qn = q.normalized();
    if (qn.w < T(0))
    {
    ...
    {
        angle = math<T>::pi;
        T mag = std::sqrt(qn.x * qn.x + qn.y * qn.y + qn.z * qn.z);
        if (mag > T(1e-6))
        {
            axis.x = qn.x / mag;
            axis.y = qn.y / mag;
