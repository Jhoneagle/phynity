...
template <typename T = float> struct Quat
{
	...
	explicit Quat(T w, T x, T y, T z) : w(w), x(x), y(y), z(z)
	{
	}
...
/// Scalar * Quaternion
template <typename T = float> inline Quat<T> operator*(T scalar, const Quat<T> &q)
{
	return q * scalar;
}

/// Dot product: q1 · q2 = w1*w2 + x1*x2 + y1*y2 + z1*z2
template <typename T = float> inline T dot(const Quat<T> &q1, const Quat<T> &q2)
{
	return q1.w * q2.w + q1.x * q2.x + q1.y * q2.y + q1.z * q2.z;
}

/// Stream output operator
template <typename T = float> inline std::ostream &operator<<(std::ostream &os, const Quat<T> &q)
{
