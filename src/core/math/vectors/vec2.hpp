...
template <typename T = float> inline Vec2<T> operator*(T scalar, const Vec2<T> &v)
{
	return v * scalar;
}

template <typename T = float> inline std::ostream &operator<<(std::ostream &os, const Vec2<T> &v)
{
	os << "(" << v.x << ", " << v.y << ")";
	return os;
}
