...
template <typename T = float> struct Vec3
{
    ...
    T angle(const Vec3 &other) const
    {
        ...
        if (denom < T(1e-6))
            return T(0); // Handle zero-length vectors
...
template <typename T = float> inline Vec3<T> operator*(T scalar, const Vec3<T> &v)
{
    return v * scalar;
}

template <typename T = float> inline std::ostream &operator<<(std::ostream &os, const Vec3<T> &v)
{
    os << "(" << v.x << ", " << v.y << ", " << v.z << ")";
    return os;
}
