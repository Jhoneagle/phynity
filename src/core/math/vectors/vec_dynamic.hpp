...
template <typename T = float> class VecDynamic
{
...
public:
	...
	template <typename Iter> VecDynamic(Iter begin, Iter end) : data(begin, end)
	{
	}
	...
	{
		if (i >= size())
			throw std::out_of_range("VecDynamic index out of range");
		return data[i];
	}
...
template <typename T = float> inline VecDynamic<T> operator*(T scalar, const VecDynamic<T> &v)
{
	return v * scalar;
}

// Stream output
template <typename T = float> inline std::ostream &operator<<(std::ostream &os, const VecDynamic<T> &v)
{
	os << "(";
