...
template <typename T = float> struct Mat2
{
	...
	constexpr Mat2(T m00, T m01, T m10, T m11)
	{
		m[0][0] = m00;
		m[0][1] = m01;
		m[1][0] = m10;
	...
	{
		return Mat2(m[0][0] + other.m[0][0], m[0][1] + other.m[0][1], m[1][0] + other.m[1][0], m[1][1] + other.m[1][1]);
	}
	...
	constexpr Mat2 operator*(const Mat2 &other) const
	{
		return Mat2(m[0][0] * other.m[0][0] + m[0][1] * other.m[1][0],
	...
	Mat2 inverse() const
	{
		...
		T invDet = T(1) / det;
		return Mat2(m[1][1] * invDet, -m[0][1] * invDet, -m[1][0] * invDet, m[0][0] * invDet);
	}
	...
	{
		return std::abs(m[0][0] - other.m[0][0]) < epsilon && std::abs(m[0][1] - other.m[0][1]) < epsilon &&
		       std::abs(m[1][0] - other.m[1][0]) < epsilon && std::abs(m[1][1] - other.m[1][1]) < epsilon;
	}

	/// Return matrix with absolute values of all elements
	Mat2 abs() const
	{
		return Mat2(std::abs(m[0][0]), std::abs(m[0][1]), std::abs(m[1][0]), std::abs(m[1][1]));
	}
...
template <typename T = float> inline Mat2<T> operator*(T scalar, const Mat2<T> &m)
{
	return m * scalar;
}

/// Vector * Matrix multiplication (treats vector as row vector)
template <typename T = float> inline Vec2<T> operator*(const Vec2<T> &v, const Mat2<T> &m)
{
	return Vec2<T>(v.x * m(0, 0) + v.y * m(1, 0), v.x * m(0, 1) + v.y * m(1, 1));
}

template <typename T = float> inline std::ostream &operator<<(std::ostream &os, const Mat2<T> &m)
{
	os << "[(" << m(0, 0) << ", " << m(0, 1) << "), "
	   << "(" << m(1, 0) << ", " << m(1, 1) << ")]";
	return os;
