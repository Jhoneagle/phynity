...
template <typename T = float> struct Mat4
{
    ...
    T determinant() const
    {
        ...
        T m00 = m[1][1] * (m[2][2] * m[3][3] - m[2][3] * m[3][2]) - m[1][2] * (m[2][1] * m[3][3] - m[2][3] * m[3][1]) +
                m[1][3] * (m[2][1] * m[3][2] - m[2][2] * m[3][1]);

        T m01 = m[1][0] * (m[2][2] * m[3][3] - m[2][3] * m[3][2]) - m[1][2] * (m[2][0] * m[3][3] - m[2][3] * m[3][0]) +
                m[1][3] * (m[2][0] * m[3][2] - m[2][2] * m[3][0]);

        T m02 = m[1][0] * (m[2][1] * m[3][3] - m[2][3] * m[3][1]) - m[1][1] * (m[2][0] * m[3][3] - m[2][3] * m[3][0]) +
                m[1][3] * (m[2][0] * m[3][1] - m[2][1] * m[3][0]);

        T m03 = m[1][0] * (m[2][1] * m[3][2] - m[2][2] * m[3][1]) - m[1][1] * (m[2][0] * m[3][2] - m[2][2] * m[3][0]) +
    ...
    {
        return Mat4(T(1), T(0), T(0), T(0), T(0), T(1), T(0), T(0), T(0), T(0), T(1), T(0), T(0), T(0), T(0), T(1));
    }

    /// Create zero matrix
    static Mat4 zero()
    {
        return Mat4(T(0), T(0), T(0), T(0), T(0), T(0), T(0), T(0), T(0), T(0), T(0), T(0), T(0), T(0), T(0), T(0));
    }

    /// Create translation matrix
    static Mat4 translation(T tx, T ty, T tz)
    {
        return Mat4(T(1), T(0), T(0), tx, T(0), T(1), T(0), ty, T(0), T(0), T(1), tz, T(0), T(0), T(0), T(1));
    }
    ...
    {
        return Mat4(sx, T(0), T(0), T(0), T(0), sy, T(0), T(0), T(0), T(0), sz, T(0), T(0), T(0), T(0), T(1));
    }
    ...
    static Mat4 rotationX(T angleRadians)
    {
        ...
        return Mat4(T(1), T(0), T(0), T(0), T(0), c, -s, T(0), T(0), s, c, T(0), T(0), T(0), T(0), T(1));
    }
    ...
    static Mat4 rotationY(T angleRadians)
    {
        T c = std::cos(angleRadians);
        T s = std::sin(angleRadians);
        return Mat4(c, T(0), s, T(0), T(0), T(1), T(0), T(0), -s, T(0), c, T(0), T(0), T(0), T(0), T(1));
    }
    ...
    static Mat4 rotationZ(T angleRadians)
    {
        T c = std::cos(angleRadians);
        T s = std::sin(angleRadians);
        return Mat4(c, -s, T(0), T(0), s, c, T(0), T(0), T(0), T(0), T(1), T(0), T(0), T(0), T(0), T(1));
    }
    ...
    static Mat4 rotationAxis(const Vec3<T> &axis, T angleRadians)
    {
        ...
        return Mat4(c + axis.x * axis.x * omc,
...
template <typename T = float> inline Mat4<T> operator*(T scalar, const Mat4<T> &m)
{
    return m * scalar;
}

/// Vector * Matrix multiplication (treats vector as row vector)
template <typename T = float> inline Vec4<T> operator*(const Vec4<T> &v, const Mat4<T> &m)
{
...
template <typename T = float> inline Vec4<T> operator*(const Mat4<T> &m, const Vec4<T> &v)
{
    return m * v;
}

/// Matrix * Vector multiplication (treats vector as column vector, w=1) - for Vec3
template <typename T = float> inline Vec3<T> operator*(const Mat4<T> &m, const Vec3<T> &v)
{
    return m * v;
}

template <typename T = float> inline std::ostream &operator<<(std::ostream &os, const Mat4<T> &m)
{
    os << "[\n";
    for (int i = 0; i < 4; ++i)
    {
