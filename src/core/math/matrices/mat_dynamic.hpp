...
template <typename T> class MatRowView
{
    template <typename U> friend struct MatDynamic;
...
template <typename T> class MatColView
{
    template <typename U> friend struct MatDynamic;
...
template <typename T = float> struct MatDynamic
{
    ...
    MatDynamic(std::size_t r, std::size_t c, T scalar) : rows(r), cols(c), data(r * c, scalar)
    {
    }
...
template <typename T = float> inline MatDynamic<T> operator*(T scalar, const MatDynamic<T> &m)
{
    return m * scalar;
}

/// Stream output
template <typename T = float> inline std::ostream &operator<<(std::ostream &os, const MatDynamic<T> &m)
{
    os << "[";
