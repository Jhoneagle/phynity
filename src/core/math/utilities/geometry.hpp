 ...
 */
template <typename T>
inline bool ray_plane_intersection(const Vec3<T> &ray_origin,
                                   ...
                                   T max_t = infinity<T>()) noexcept
{
	...
	T denom = plane.a * ray_direction.x + plane.b * ray_direction.y + plane.c * ray_direction.z;
	...
	T numerator = -(plane.a * ray_origin.x + plane.b * ray_origin.y + plane.c * ray_origin.z + plane.d);
 ...
 */
template <typename T> inline bool point_in_sphere(const Vec3<T> &point, const Sphere<T> &sphere) noexcept
{
	static_assert(std::is_floating_point_v<T>);
...
template <typename T>
inline bool
point_on_sphere(const Vec3<T> &point, const Sphere<T> &sphere, T tolerance = geometry<T>::contact_epsilon) noexcept
{
	static_assert(std::is_floating_point_v<T>);
 ...
 */
template <typename T> inline bool sphere_sphere_intersection(const Sphere<T> &s1, const Sphere<T> &s2) noexcept
{
	static_assert(std::is_floating_point_v<T>);
 ...
 */
template <typename T> inline Vec3<T> closest_point_on_sphere(const Vec3<T> &point, const Sphere<T> &sphere) noexcept
{
	static_assert(std::is_floating_point_v<T>);
	...
	return Vec3<T>(sphere.center.x + dx * scale, sphere.center.y + dy * scale, sphere.center.z + dz * scale);
}
 ...
 */
template <typename T> inline bool point_in_aabb(const Vec3<T> &point, const AABB<T> &aabb) noexcept
{
 ...
 */
template <typename T> inline bool aabb_aabb_intersection(const AABB<T> &a1, const AABB<T> &a2) noexcept
{
	return a1.min.x <= a2.max.x && a1.max.x >= a2.min.x && a1.min.y <= a2.max.y && a1.max.y >= a2.min.y &&
 ...
 */
template <typename T> inline Vec3<T> closest_point_on_aabb(const Vec3<T> &point, const AABB<T> &aabb) noexcept
{
	return Vec3<T>(clamp(point.x, aabb.min.x, aabb.max.x),
