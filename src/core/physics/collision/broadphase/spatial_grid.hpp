...
{
public:
	explicit SpatialGrid(float cell_size = 1.0f) noexcept : hash_(cell_size)
	{
	}
	 ...
	 */
	[[nodiscard]] std::vector<uint32_t> get_objects_in_cell(const math::vectors::Vec3f &position) const noexcept
	{
		const auto cell_hash = hash_.hash_position(position);
			...
			for (int dy = -1; dy <= 1; ++dy)
			{
				for (int dz = -1; dz <= 1; ++dz)
				{
					const auto neighbor_cell =
					    GridCell{safe_add(cell.x, dx), safe_add(cell.y, dy), safe_add(cell.z, dz)};
					const auto neighbor_hash =
					    SpatialGridHash::hash_coords(neighbor_cell.x, neighbor_cell.y, neighbor_cell.z);
