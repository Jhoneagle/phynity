...
RaySpherIntersection LinearCast::raycast_sphere(const Vec3f &ray_origin,
                                                               ...
                                                               float max_time)
{
    ...
    if (speed < 1e-10f)
    {
        ...
        if (distance < combined_radius)
        {
            ...
            result.t_near = 0.0f;
            result.t_far = 0.0f;
            result.contact_point = static_sphere_center + (distance > 1e-6f ? diff.normalized() * static_sphere_radius
