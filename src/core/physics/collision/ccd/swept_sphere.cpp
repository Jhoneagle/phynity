...
TimeOfImpactResult SweptSphereSolver::solve_static(
    ...
    if (current_distance < min_distance) {
        ...
        Vec3f normal = (current_distance > 1e-6f) 
    ...
    if (!LinearCast::are_spheres_approaching(
        sphere_a.position, sphere_a.velocity,
        sphere_b.position, sphere_b.velocity
    )) {
...
Vec3f SweptSphereSolver::compute_contact_point(
    float time,
    const Sphere& sphere_a,
    const Sphere& sphere_b
) {
...
Vec3f SweptSphereSolver::compute_contact_normal(
    float time,
    const Sphere& sphere_a,
    const Sphere& sphere_b
) {
...
Vec3f SweptSphereSolver::compute_relative_velocity(
    const Sphere& sphere_a,
    const Sphere& sphere_b
) {
