...
public:
    ...
    explicit TimestepController(float target_timestep = 1.0f / 60.0f,
    ...
    float step()
    {
        ...
        if (overflow_mode_ == OverflowMode::CLAMP && stats_.overflow_count > 0)
        {
            ...
            if (consumed >= max_timestep_ && accumulated_time_ > TIMESTEP_EPSILON &&
                accumulated_time_ < target_timestep_ - TIMESTEP_EPSILON)
            {
                accumulated_time_ = 0.0f;
            }
        }
    ...
    template <typename Callback> int step_all(Callback callback)
    {
        int steps = 0;
        while (true)
        {
