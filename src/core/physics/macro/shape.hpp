...
public:
    ...
    explicit Shape(ShapeType type, const Vec3f &center = Vec3f(0.0f)) : shape_type(type), local_center(center)
    {
    }
    ...
    virtual void compute_inertia_tensors(float mass, Mat3f &I, Mat3f &I_inv) const = 0;
...
public:
    ...
    void compute_inertia_tensors(float mass, Mat3f &I, Mat3f &I_inv) const override
    {
        // Sphere: I = (2/5) * m * r²
...
public:
    ...
    void compute_inertia_tensors(float mass, Mat3f &I, Mat3f &I_inv) const override
    {
        ...
        I_inv = Mat3f(0.0f);
        if (diag_xx > 1e-6f)
            I_inv(0, 0) = 1.0f / diag_xx;
        if (diag_yy > 1e-6f)
            I_inv(1, 1) = 1.0f / diag_yy;
        if (diag_zz > 1e-6f)
            I_inv(2, 2) = 1.0f / diag_zz;
    }
