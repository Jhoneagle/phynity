...
private:
    ...
    static void apply_contact_impulse(
        RigidBody& body_a,
        RigidBody& body_b,
        const collision::ContactPoint& contact
    ) {
        ...
        for (size_t i = 0; i + 1 < bodies_.size(); ++i) {
            for (size_t j = i + 1; j < bodies_.size(); ++j) {
                ...
                if (end_overlap && sphere_sphere) {
                    ...
                    collision::SphereCollider collider_a{
                        body_a.position + sphere_a->local_center,
                        body_a.velocity,
                        body_a.inv_mass,
                    ...
                    collision::SphereCollider collider_b{
                        body_b.position + sphere_b->local_center,
                        body_b.velocity,
                        body_b.inv_mass,
                ...
                if (!manifold.is_valid() && config_.enable_linear_ccd && dt > 1e-8f) {
                    ...
                    if (!manifold.is_valid() && config_.ccd_config.enable_rotational_ccd && sphere_sphere) {
                        ...
                        collision::ccd::TimeOfImpactResult toi =
                            collision::ccd::ConservativeAdvancement::solve_sphere_sphere(
                                sphere_a,
                                sphere_b,
                                dt
                            );
                ...
                if (!manifold.is_valid() &&
                    config_.enable_linear_ccd &&
                    config_.ccd_config.use_speculative_contacts &&
