...
struct SerializedState
{
    ...
    bool equals(const SerializedState &other, float tolerance = 1e-6f) const
    {
        ...
        if (positions.size() != other.positions.size())
            return false;
        if (velocities.size() != other.velocities.size())
            return false;
...
inline SerializedState
snapshot_system(const ParticleSystem &system, uint64_t frame_number = 0, float elapsed_time = 0.0f)
{
