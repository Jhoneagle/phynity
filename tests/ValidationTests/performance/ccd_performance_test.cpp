...
TEST_CASE("CCD Performance: Particle system overhead", "[validation][performance][ccd]") {
    ...
    SECTION("Measure CCD overhead for particles") {
        ...
        PerfResult conservative = benchmark_particle_ccd(particle_count, frames, ccd_presets::conservative(), "conservative");
        ...
        std::cout << "  Conservative: " << conservative.milliseconds << " ms (overhead: "
                  << (conservative.milliseconds / disabled.milliseconds - 1.0) * 100.0 << "%)\n";
        std::cout << "  Balanced:     " << balanced.milliseconds << " ms (overhead: "
                  << (balanced.milliseconds / disabled.milliseconds - 1.0) * 100.0 << "%)\n";

        // Sanity check: CCD should not cause dramatic slowdown (>3x)
        // (The optimization goal is to keep overhead reasonable)
        REQUIRE(conservative.milliseconds < disabled.milliseconds * 3.0);
        REQUIRE(balanced.milliseconds < disabled.milliseconds * 3.0);
    }
}
...
TEST_CASE("CCD Performance: Rigid body overhead", "[validation][performance][ccd]") {
    ...
    SECTION("Measure CCD overhead for rigid bodies") {
        ...
        std::cout << "  Disabled: " << disabled.milliseconds << " ms\n";
        std::cout << "  Enabled:  " << enabled.milliseconds << " ms (overhead: "
                  << (enabled.milliseconds / disabled.milliseconds - 1.0) * 100.0 << "%)\n";
...
TEST_CASE("CCD Performance: Scaling with particle count", "[validation][performance][ccd]") {
    ...
    SECTION("Measure how CCD overhead scales") {
        ...
        std::cout << "  50 particles:  " << small_off.milliseconds << " ms -> " << small_on.milliseconds 
                  << " ms (+" << small_overhead_pct << "%)\n";
        std::cout << "  200 particles: " << large_off.milliseconds << " ms -> " << large_on.milliseconds 
