...
TEST_CASE("Pendulum - energy conservation", "[integration_harness]")
{
    ...
    while (state.time < max_time)
    {
        ...
        float kinetic = 0.5f * problem.mass * problem.length * problem.length * state.velocity[0] * state.velocity[0];
        float potential = problem.mass * problem.gravity * problem.length * (1.0f - std::cos(state.position[0]));
