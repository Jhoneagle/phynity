...
BounceMetrics measure_single_bounce(
    ParticleSystem &system, size_t particle_index, float dt, int max_steps, float impact_height_threshold)
{
    ...
    for (int i = 0; i < max_steps; ++i)
    {
        ...
        if (!metrics.impact_detected && prev_vy < 0.0f && p.velocity.y > 0.0f && p.position.y < impact_height_threshold)
        {
...
TEST_CASE("Restitution: coefficient sweep", "[material][restitution][validation]")
{
    ...
    for (float e : coefficients)
    {
        SECTION("e=" + std::to_string(e))
        {
            ...
            float measured_e = (metrics.impact_speed > 0.0f) ? (metrics.rebound_speed / metrics.impact_speed) : 0.0f;
