...
TEST_CASE("PGS - Multi-iteration convergence behavior", "[collision][pgs][golden]")
{
    ...
    SECTION("1 iteration vs 4 iterations")
    {
        ...
        for (size_t i = 0; i < 4; ++i)
        {
            if (colliders_1iter[i].velocity.length() > 1e-6f)
                has_velocity_1iter = true;
            if (colliders_4iter[i].velocity.length() > 1e-6f)
                has_velocity_4iter = true;
        }
...
TEST_CASE("PGS - Warm-start initialization", "[collision][pgs][golden]")
{
    ...
    REQUIRE((colliders_no_ws[0].velocity.length() > 1e-6f || colliders_no_ws[1].velocity.length() > 1e-6f));
}
