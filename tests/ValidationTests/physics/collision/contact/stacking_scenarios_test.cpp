...
void wait_until_settled(ParticleSystem &system,
...
TEST_CASE("Stacking: Fixed constraint maintains distance", "[stacking][validation]")
{
	...
	float initial_distance = (system.particles()[p2_idx].position - system.particles()[p1_idx].position).length();
...
TEST_CASE("Stacking: Pyramid structure (loose bounds)", "[stacking][validation]")
{
	...
	for (int x = -1; x <= 1; ++x)
	{
		for (int z = -1; z <= 1; ++z)
		{
			system.spawn(Vec3f(static_cast<float>(x) * spacing, 1.0f, static_cast<float>(z) * spacing),
			             Vec3f(0, 0, 0),
			             mat,
			             -1.0f,
			             0.5f);
