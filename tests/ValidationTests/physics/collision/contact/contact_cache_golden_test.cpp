...
TEST_CASE("ContactCache - Stacked particles persistence", "[collision][contact_cache][golden]")
{
    ...
    // Add gravity
    system.add_force_field(std::make_unique<phynity::physics::GravityField>(Vec3f(0.0f, -9.8f, 0.0f) // gravity
                                                                            ));
    ...
    SECTION("Determinism - Same initial conditions produce same results")
    {
        ...
        for (auto *sys : {&system1, &system2})
        {
            ...
            sys->add_force_field(std::make_unique<phynity::physics::GravityField>(Vec3f(0.0f, -9.8f, 0.0f)));
        }
