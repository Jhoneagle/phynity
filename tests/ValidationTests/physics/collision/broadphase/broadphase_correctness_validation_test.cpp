    ...
    for (const auto &manifold : manifolds)
    {
        if (manifold.object_a_id < particles.size() && manifold.object_b_id < particles.size())
        {
            constraints.push_back(
                std::make_unique<constraints::ContactConstraint>(manifold,
...
TEST_CASE("Broadphase correctness: Dense cluster collision", "[validation][broadphase]")
{
    ...
    SECTION("4-particle square cluster")
    {
        ...
        const std::vector<Vec3f> positions = {
            Vec3f(-0.5f, -0.5f, 0.0f), Vec3f(0.5f, -0.5f, 0.0f), Vec3f(-0.5f, 0.5f, 0.0f), Vec3f(0.5f, 0.5f, 0.0f)};

        const std::vector<Vec3f> velocities = {Vec3f(0.5f, 0.5f, 0.0f), // Moving toward center
...
TEST_CASE("Broadphase correctness: Conservation laws", "[validation][broadphase]")
{
    ...
    SECTION("Momentum conservation in collision")
    {
        ...
        REQUIRE_THAT(initial_diag.total_momentum.x, Catch::Matchers::WithinAbs(0.0f, momentum_tolerance));
        ...
        REQUIRE_THAT(final_diag.total_momentum.x, Catch::Matchers::WithinAbs(0.0f, momentum_tolerance));
    }
}
...
TEST_CASE("Broadphase correctness: Different grid cell sizes", "[validation][broadphase]")
{
    ...
    SECTION("Same scenario with different cell sizes")
    {
        ...
        {
            REQUIRE_THAT(coarse[i].position.x, Catch::Matchers::WithinAbs(fine[i].position.x, pos_tolerance * 2.0f));
