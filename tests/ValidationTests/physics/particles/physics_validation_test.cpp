...
TEST_CASE("Physics Validation - Reference-based determinism (constant accel)", "[physics_validation]")
{
    ...
    system.spawn(Vec3f(0.0f, p0, 0.0f), Vec3f(0.0f, v0, 0.0f), make_no_damping_material(1.0f));
    system.add_force_field(std::make_unique<GravityField>(Vec3f(0.0f, a, 0.0f)));
...
TEST_CASE("Physics Validation - Reference-based determinism (linear drag)", "[physics_validation]")
{
    ...
    system.spawn(Vec3f(p0, 0.0f, 0.0f), Vec3f(v0, 0.0f, 0.0f), make_no_damping_material(1.0f));
...
TEST_CASE("Physics Validation - Reference-based determinism (spring force)", "[physics_validation]")
{
    ...
    system.spawn(Vec3f(x0, 0.0f, 0.0f), Vec3f(v0, 0.0f, 0.0f), make_no_damping_material(mass));
...
TEST_CASE("Physics Validation - Momentum conservation in zero gravity", "[physics_validation]")
{
    ...
    system.spawn(Vec3f(-5.0f, 0.0f, 0.0f), Vec3f(10.0f, 0.0f, 0.0f),
                 make_no_damping_material(2.0f)); // mass=2kg
    system.spawn(Vec3f(5.0f, 0.0f, 0.0f), Vec3f(-5.0f, 0.0f, 0.0f),
                 make_no_damping_material(1.0f)); // mass=1kg
...
TEST_CASE("Physics Validation - Energy conservation with gravity", "[physics_validation]")
{
    ...
    float mass = 1.0f;
    system.spawn(Vec3f(0.0f, 50.0f, 0.0f), Vec3f(0.0f, 20.0f, 0.0f), make_no_damping_material(mass));
    system.add_force_field(std::make_unique<GravityField>(Vec3f(0.0f, -EARTH_GRAVITY, 0.0f)));
    ...
    for (int i = 0; i < frames; ++i)
    {
        ...
        const auto &p = system.particles()[0];
        float KE =
            0.5f * mass * (p.velocity.x * p.velocity.x + p.velocity.y * p.velocity.y + p.velocity.z * p.velocity.z);
...
TEST_CASE("Physics Validation - Projectile motion range", "[physics_validation]")
{
    ...
    system.spawn(Vec3f(0.0f, 0.0f, 0.0f), Vec3f(vx, vy, 0.0f), make_no_damping_material(1.0f));
    system.add_force_field(std::make_unique<GravityField>(Vec3f(0.0f, -EARTH_GRAVITY, 0.0f)));
    ...
    for (int i = 0; i < max_frames; ++i)
    {
        ...
        const auto &p = system.particles()[0];
        if (p.position.y < 0.1f && i > 100)
        { // Skip initial frames, check y ≈ 0
...
TEST_CASE("Physics Validation - Linear drag force", "[physics_validation]")
{
    ...
    system.spawn(Vec3f(0.0f, 0.0f, 0.0f), Vec3f(v0, 0.0f, 0.0f), make_no_damping_material(1.0f));
    system.add_force_field(std::make_unique<DragField>(k));
...
TEST_CASE("Physics Validation - System energy conservation (multi-particle)", "[physics_validation]")
{
    ...
    system.spawn(Vec3f(0.0f, 30.0f, 0.0f), Vec3f(0.0f, 0.0f, 0.0f), make_no_damping_material(2.0f));
    system.spawn(Vec3f(5.0f, 15.0f, 0.0f), Vec3f(-3.0f, 5.0f, 0.0f), make_no_damping_material(1.5f));
...
TEST_CASE("Physics Validation - Orbit stability in spring field", "[physics_validation]")
{
    ...
    system.spawn(Vec3f(radius, 0.0f, 0.0f), Vec3f(0.0f, 0.0f, tangential_speed), make_no_damping_material(mass));
...
TEST_CASE("Physics Validation - Simple elastic collision", "[physics_validation]")
{
    ...
    system.spawn(
        Vec3f(-1.0f, 0.0f, 0.0f), Vec3f(2.0f, 0.0f, 0.0f), make_no_damping_material(1.0f, restitution), -1.0f, radius);
    system.spawn(
        Vec3f(1.0f, 0.0f, 0.0f), Vec3f(-2.0f, 0.0f, 0.0f), make_no_damping_material(1.0f, restitution), -1.0f, radius);
