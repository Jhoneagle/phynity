...
TEST_CASE("Profiler Overhead: Nested scope scaling", "[validation][profiler][benchmark]")
{
	...
	for (int depth : depths)
	{
		...
		auto baseline_end = std::chrono::high_resolution_clock::now();
		const auto baseline_duration =
		    std::chrono::duration_cast<std::chrono::nanoseconds>(baseline_end - baseline_start);
		...
		auto profiled_end = std::chrono::high_resolution_clock::now();
		const auto profiled_duration =
		    std::chrono::duration_cast<std::chrono::nanoseconds>(profiled_end - profiled_start);

		const double overhead_ns =
		    static_cast<double>(profiled_duration.count() - baseline_duration.count()) / iterations_per_depth;
...
TEST_CASE("Profiler Overhead: Scope creation cost", "[validation][profiler][benchmark]")
{
	...
	const double scope_limit = is_slow_env() ? 2000.0 : 1000.0;
	REQUIRE(ns_per_scope < scope_limit); // Less than threshold per scope
}
...
TEST_CASE("Profiler Overhead: Multiple concurrent scopes", "[validation][profiler][benchmark]")
{
	...
	const double iteration_limit = is_slow_env() ? 10000.0 : 4000.0;
	REQUIRE(ns_per_iteration < iteration_limit);
}
