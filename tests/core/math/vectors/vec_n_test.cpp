...
TEST_CASE("VecN: Constructors", "[VecN][constructor]") {
    ...
    SECTION("Scalar constructor") {
        phynity::math::vectors::VecN<3> v(5.0f);
...
TEST_CASE("VecN: Arithmetic operations", "[VecN][arithmetic]") {
    phynity::math::vectors::VecN<4> a;
    a[0] = 1.0f; a[1] = 2.0f; a[2] = 3.0f; a[3] = 4.0f;
    
    phynity::math::vectors::VecN<4> b;
    b[0] = 5.0f; b[1] = 6.0f; b[2] = 7.0f; b[3] = 8.0f;

    SECTION("Negation") {
        phynity::math::vectors::VecN<3> v;
        v[0] = 1.0f; v[1] = -2.0f; v[2] = 3.0f;
        phynity::math::vectors::VecN<3> neg = -v;
...
TEST_CASE("VecN: Vector operations", "[VecN][operations]") {
    ...
    SECTION("Normalization") {
        phynity::math::vectors::VecN<5> n = v.normalized();
        REQUIRE_THAT(n.length(), WithinAbs(1.0f, 1e-5f));
    }
}

TEST_CASE("VecN: 6D vector (robotics use case)", "[VecN][6D]") {
    phynity::math::vectors::VecN<6> state;  // 3 linear + 3 angular
    ...
    SECTION("Operations preserve size") {
        phynity::math::vectors::VecN<6> scaled = state * 2.0f;
        REQUIRE(scaled.size() == 6);
    }

    SECTION("Static size method") {
        REQUIRE(phynity::math::vectors::VecN<6>::size() == 6);
        REQUIRE(phynity::math::vectors::VecN<3>::size() == 3);
        REQUIRE(phynity::math::vectors::VecN<16>::size() == 16);
    }
}
...
TEST_CASE("VecN: Assignment operators", "[VecN][assignment]") {
    ...
    SECTION("Component-wise multiplication assignment") {
        phynity::math::vectors::VecN<4> other;
...
TEST_CASE("VecN: Query functions", "[VecN][query]") {
    SECTION("isZero") {
        phynity::math::vectors::VecN<3> zero;
        REQUIRE(zero.isZero());
        zero[0] = 0.001f;
        REQUIRE(!zero.isZero());
    }
    ...
    SECTION("isNormalized") {
        phynity::math::vectors::VecN<3> unit;
        unit[0] = 1.0f;
    ...
    SECTION("abs") {
        ...
        phynity::math::vectors::VecN<3> result = v.abs();
...
TEST_CASE("VecN: Utility operations", "[VecN][utility]") {
    ...
    SECTION("Distance") {
        phynity::math::vectors::VecN<3> other;
    ...
    SECTION("Lerp") {
        ...
        phynity::math::vectors::VecN<3> mid = a.lerp(b, 0.5f);
    ...
    SECTION("Min/Max") {
        ...
        phynity::math::vectors::VecN<3> minV = a.min(b);
        phynity::math::vectors::VecN<3> maxV = a.max(b);
...
TEST_CASE("VecN: Edge cases", "[VecN][edge]") {
    SECTION("Division by zero scalar") {
        phynity::math::vectors::VecN<3> v;
        v[0] = 1.0f; v[1] = 2.0f; v[2] = 3.0f;
        phynity::math::vectors::VecN<3> result = v / 0.0f;
    ...
    SECTION("Large dimension vector") {
        phynity::math::vectors::VecN<100> v(1.0f);
        REQUIRE(v.size() == 100);
