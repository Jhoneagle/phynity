...
// ============================================================================
// QUATERNION TO EULER ANGLES TESTS
// ============================================================================

/// Compare two Vec3 values with tolerance
bool vec3Equal(const Vec3& v1, const Vec3& v2, float tolerance = 1e-5f) {
    return (std::abs(v1.x - v2.x) < tolerance) &&
           (std::abs(v1.y - v2.y) < tolerance) &&
           (std::abs(v1.z - v2.z) < tolerance);
}

Quat eulerAnglesToQuat(const Vec3& euler) {
    float roll  = euler.x;
    float pitch = euler.y;
    float yaw   = euler.z;
...
TEST_CASE("Conversion: Identity quaternion to zero Euler angles", "[conversion][quat-to-euler]") {
    // Construct quaternion for roll=0, pitch=0, yaw=0 using ZYX formula
    Vec3 expected(0.0f, 0.0f, 0.0f);
    Quat q = eulerAnglesToQuat(expected);
    Vec3 euler = toEulerAngles(q);
    
    REQUIRE_THAT(euler.x, WithinAbs(expected.x, 1e-5f));
    REQUIRE_THAT(euler.y, WithinAbs(expected.y, 1e-5f));
    REQUIRE_THAT(euler.z, WithinAbs(expected.z, 1e-5f));
}

TEST_CASE("Conversion: 90° rotation around X-axis (quat to euler)", "[conversion][quat-to-euler]") {
    // Construct quaternion for roll=90°, pitch=0, yaw=0 using ZYX formula
    Vec3 expected(mathf::pi / 2.0f, 0.0f, 0.0f);
    Quat q = eulerAnglesToQuat(expected);
    Vec3 euler = toEulerAngles(q);
...
TEST_CASE("Conversion: Pure pitch = 45° (quat to euler)", "[conversion][quat-to-euler]") {
    // Construct quaternion for roll=0, pitch=45°, yaw=0 using ZYX formula
    Vec3 expected(0.0f, mathf::pi / 4.0f, 0.0f);
    Quat q = eulerAnglesToQuat(expected);
    Vec3 euler = toEulerAngles(q);
...
TEST_CASE("Conversion: 90° rotation around Z-axis (quat to euler)", "[conversion][quat-to-euler]") {
    // Construct quaternion for roll=0, pitch=0, yaw=90° using ZYX formula
    Vec3 expected(0.0f, 0.0f, mathf::pi / 2.0f);
    Quat q = eulerAnglesToQuat(expected);
    Vec3 euler = toEulerAngles(q);
