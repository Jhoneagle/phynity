...
TEST_CASE("ScopedTimer: Multiple measurements", "[diagnostics][timer]")
{
    ...
    const uint64_t upper_bound = is_slow_env() ? 200000 : 100000;
    REQUIRE(duration1 < upper_bound); // Less than upper bound
    REQUIRE(duration2 < upper_bound); // Less than upper bound
}
