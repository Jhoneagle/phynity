...
TEST_CASE("FrameProfiler: Average frame time", "[diagnostics][frame_profiler]")
{
    ...
    const uint64_t avg_upper = is_slow_env() ? 80000 : 30000;
    REQUIRE(avg <= avg_upper); // Upper bound varies by environment
...
TEST_CASE("FrameProfiler: Multiple zones per frame", "[diagnostics][frame_profiler]")
{
    ...
    const uint64_t zone_sum = frame.zones[0].duration_us + frame.zones[1].duration_us + frame.zones[2].duration_us;
