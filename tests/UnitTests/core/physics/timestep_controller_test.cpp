...
TEST_CASE("TimestepController: Step returns zero before accumulated_time reaches target", "[TimestepController][step]")
{
	TimestepController controller(0.016f);
...
TEST_CASE("TimestepController: Step returns target_timestep when enough accumulated", "[TimestepController][step]")
{
	TimestepController controller(0.016f);
...
TEST_CASE("TimestepController: SUBDIVIDE mode clamps and tracks overflow", "[TimestepController][overflow][subdivide]")
{
...
TEST_CASE("TimestepController: step_all executes callback for each step", "[TimestepController][step_all]")
{
	...
	int steps = controller.step_all([&timesteps](float dt) { timesteps.push_back(dt); });
...
TEST_CASE("TimestepController: Fixed timestep produces deterministic steps", "[TimestepController][determinism]")
{
...
TEST_CASE("TimestepController: Frame rate spike handling (CLAMP mode)", "[TimestepController][scenarios]")
{
	...
	while (controller.step() > 0.0f)
		steps++;
	(void) steps; // Variable used for counting iterations but value not needed
