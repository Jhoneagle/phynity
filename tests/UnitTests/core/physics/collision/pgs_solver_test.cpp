...
TEST_CASE("PGSSolver - Multi-iteration reduces penetration faster", "[collision][pgs]")
{
    ...
    float vel_diff_1iter = colliders_1iter[0].velocity.length() + colliders_1iter[1].velocity.length();
    float vel_diff_4iter = colliders_4iter[0].velocity.length() + colliders_4iter[1].velocity.length();
...
TEST_CASE("PGSSolver - No friction with zero coefficient", "[collision][pgs]")
{
    ...
    REQUIRE_THAT(colliders_no_friction[0].velocity.x, WithinAbs(vel_before.x, 1e-5f));
}
