...
TEST_CASE("SAT2D - Two separated boxes do not collide", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_2d(box_a, box_b, Vec2f(0.0f), Vec2f(5.0f));
...
TEST_CASE("SAT2D - Two overlapping boxes collide", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_2d(box_a, box_b, Vec2f(0.0f), Vec2f(0.5f));
...
TEST_CASE("SAT2D - Touching boxes (edge-to-edge) do not collide", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_2d(box_a, box_b, Vec2f(0.0f), Vec2f(2.0f));
...
TEST_CASE("SAT2D - Rotated box vs axis-aligned box", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_2d(box_a, diamond, Vec2f(0.0f), Vec2f(0.0f));
...
TEST_CASE("SAT2D - Two separate polygons", "[collision][sat]")
{
    ...
    // Move hex_b far away
    SATCollisionResult result = SATSolver::test_convex_hull_2d(hex_a, hex_b, Vec2f(0.0f), Vec2f(10.0f, 0.0f));
...
{
    auto tri_a = ShapeFactory::create_triangle_2d(Vec2f(0.0f, 0.0f), Vec2f(2.0f, 0.0f), Vec2f(1.0f, 2.0f));

    auto tri_b = ShapeFactory::create_triangle_2d(Vec2f(1.0f, 0.0f), Vec2f(3.0f, 0.0f), Vec2f(2.0f, 2.0f));

    SATCollisionResult result = SATSolver::test_convex_hull_2d(tri_a, tri_b, Vec2f(0.0f), Vec2f(0.0f));
...
TEST_CASE("SAT2D - Collision normal points from A to B", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_2d(box_a, box_b, Vec2f(-2.0f, 0.0f), Vec2f(-0.5f, 0.0f));
...
TEST_CASE("SAT2D - Penetration depth is positive", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_2d(box_a, box_b, Vec2f(0.0f), Vec2f(0.3f, 0.0f));
...
TEST_CASE("SAT3D - Two separated boxes", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_3d(box_a, box_b, Vec3f(0.0f), Vec3f(5.0f));
...
TEST_CASE("SAT3D - Two overlapping boxes", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_3d(box_a, box_b, Vec3f(0.0f), Vec3f(0.5f));
...
TEST_CASE("SAT3D - Box vs tetrahedron overlapping", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_3d(box, tet, Vec3f(0.0f), Vec3f(0.0f));
...
TEST_CASE("SAT3D - Box vs cylinder", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_3d(box, cyl, Vec3f(0.0f), Vec3f(0.0f));
...
TEST_CASE("SAT2D - Deterministic results", "[collision][sat][determinism]")
{
    ...
    {
        REQUIRE_THAT(result1.penetration_depth, WithinAbs(result2.penetration_depth, tolerance::STRICT));
        REQUIRE_THAT(result1.collision_normal.x, WithinAbs(result2.collision_normal.x, tolerance::STRICT));
        REQUIRE_THAT(result1.collision_normal.y, WithinAbs(result2.collision_normal.y, tolerance::STRICT));
    }
}
...
TEST_CASE("SAT2D - Identical boxes at same position", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_2d(box, box, Vec2f(0.0f), Vec2f(0.0f));
...
TEST_CASE("SAT3D - Deeply overlapping boxes", "[collision][sat]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_3d(box_a, box_b, Vec3f(0.0f), Vec3f(0.0f));
...
TEST_CASE("SAT2D - Golden scenario: axis alignment", "[collision][sat][golden]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_2d(box_a, box_b, Vec2f(-1.0f, 0.0f), Vec2f(0.5f, 0.0f));
...
TEST_CASE("SAT2D - Golden scenario: 45-degree rotation", "[collision][sat][golden]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_2d(box_axis, box_rot, Vec2f(0.0f), Vec2f(0.0f));
...
TEST_CASE("SAT2D - Golden scenario: edge touching", "[collision][sat][golden]")
{
    ...
    SATCollisionResult result = SATSolver::test_convex_hull_2d(box_a, box_b, Vec2f(-2.0f, 0.0f), Vec2f(2.0f, 0.0f));
