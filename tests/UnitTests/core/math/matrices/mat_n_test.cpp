...
TEST_CASE("MatN: determinant and inverse (small N)", "[MatN][determinant][inverse]")
{
    ...
    {
        MatN<3, 3> m(std::array<float, 9>{2, 0, 0, 0, 3, 0, 0, 0, 4});
        REQUIRE_THAT(m.determinant(), WithinAbs(24.0f, 1e-5f));
    }

    SECTION("4x4 determinant")
    {
        MatN<4, 4> m(std::array<float, 16>{2, 0, 0, 0, 0, 3, 0, 0, 0, 0, 4, 0, 0, 0, 0, 5});
...
{
    MatN<3, 3> m(std::array<float, 9>{1, 2, 3, 0, 4, 5, 1, 0, 6});
