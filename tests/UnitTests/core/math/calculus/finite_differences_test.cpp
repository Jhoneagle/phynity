...
TEST_CASE("Finite Differences: Cubic polynomial", "[calculus][finite_differences]")
{
    ...
    {
        auto f_d = [](double xd) { return xd * xd * xd + 2.0 * xd * xd - 5.0 * xd + 1.0; };
        auto f_prime2_d = [](double xd) { return 6.0 * xd + 4.0; };
        const double xd = 2.0;
        const double deriv2 = central_difference_second(f_d, xd, 1e-4);
        REQUIRE_THAT(deriv2, WithinAbs(f_prime2_d(xd), 0.5));
    }
}
...
TEST_CASE("Finite Differences: Vector-valued function", "[calculus][finite_differences]")
{
    ...
    // f'(x) = [cos(x), -sin(x)]
    auto f = [](float x) -> VecN<2, float> { return VecN<2, float>(std::array<float, 2>{std::sin(x), std::cos(x)}); };
